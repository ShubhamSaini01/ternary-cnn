...
int main(int argc, char** argv) {
    ...
    EngineDNNL_F32 engine;
    engine.init(m, scales_path);

    if (test_path) {
        printf("\n══════════════════════════════════════════════\n");
        printf("ACCURACY TEST (oneDNN FP32 engine)\n");
        printf("══════════════════════════════════════════════\n");
        TestData td = load_cifar10_batch(test_path);
        if (td.images.empty()) { fprintf(stderr, "No test data\n"); return 1; }
        ...
        for (size_t i = 0; i < num_test; i++) {
            auto output = engine.forward(td.images[i]);
            if (argmax(output) == td.labels[i]) correct++;
            if ((i + 1) % 500 == 0 || i + 1 == num_test)
                printf("  %zu/%zu  acc=%.2f%%\n", i + 1, num_test, 100.0 * correct / (i + 1));
        }
        printf("\nOverall: %d/%zu = %.2f%%\n", correct, num_test, 100.0 * correct / num_test);
    }

    printf("\n══════════════════════════════════════════════\n");
    printf("LATENCY BENCHMARK (oneDNN FP32 engine)\n");
    printf("══════════════════════════════════════════════\n");

    std::vector<float> dummy(3 * 32 * 32, 0.1f);

    printf("Warming up (%d runs)...\n", warmup);
    for (int i = 0; i < warmup; i++) engine.forward(dummy);
