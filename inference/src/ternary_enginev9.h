...
// static void im2col_rowmajor(const Tensor& input,int ni,
//     int kH,int kW,int stride,int padding,int out_h,int out_w,float* col){

//     int in_c=input.c,in_h=input.h,in_w=input.w;
//     int cc=out_h*out_w;
//     int idx=0;
...
//                     if(ih < 0 || ih >= in_h){
//                         for(int kw=0;kw<kW;kw++) col[idx++] = 0.0f;
//                         continue;
//                     }

//                     const float* row_ptr = ch + ih*in_w;

//                     for(int kw=0;kw<kW;kw++){
//                         int iw = iw_base + kw;
//                         col[idx++] = (iw>=0 && iw<in_w) ? row_ptr[iw] : 0.0f;
//                     }
//                 }
//             }
//         }
        ...
        for (uint32_t oc = 0; oc < p.out_channels; oc++) {
            ...
            while (pi < (int)pos.size()) {
                ...
                for (int j = 0; j+15 < cc16; j+=16) {
                    ...
                    int ci = pi;
                    for (; ci + 3 < end; ci += 4) {
                        const int8_t* base0 = col_i8.data() + pos[ci+0]*cc16 + j;
                        const int8_t* base1 = col_i8.data() + pos[ci+1]*cc16 + j;
                        const int8_t* base2 = col_i8.data() + pos[ci+2]*cc16 + j;
                        const int8_t* base3 = col_i8.data() + pos[ci+3]*cc16 + j;

                        __m128i b0 = _mm_loadu_si128((__m128i*)base0);
                        __m128i b1 = _mm_loadu_si128((__m128i*)base1);
                        __m128i b2 = _mm_loadu_si128((__m128i*)base2);
                        __m128i b3 = _mm_loadu_si128((__m128i*)base3);

                        s0 = _mm256_add_epi16(s0, _mm256_cvtepi8_epi16(b0));
                        s1 = _mm256_add_epi16(s1, _mm256_cvtepi8_epi16(b1));
                        s2 = _mm256_add_epi16(s2, _mm256_cvtepi8_epi16(b2));
                        s3 = _mm256_add_epi16(s3, _mm256_cvtepi8_epi16(b3));
                    }

                    __m256i s16 = _mm256_add_epi16(_mm256_add_epi16(s0, s1),
                                                  _mm256_add_epi16(s2, s3));
            ...
            while (nni < (int)neg.size()) {
                ...
                for (int j = 0; j+15 < cc16; j+=16) {
                    ...
                    int ci = nni;
                    for (; ci + 3 < end; ci += 4) {
                        __m128i b0 = _mm_loadu_si128((__m128i*)(col_i8.data() + neg[ci+0]*cc16 + j));
                        __m128i b1 = _mm_loadu_si128((__m128i*)(col_i8.data() + neg[ci+1]*cc16 + j));
                        __m128i b2 = _mm_loadu_si128((__m128i*)(col_i8.data() + neg[ci+2]*cc16 + j));
                        __m128i b3 = _mm_loadu_si128((__m128i*)(col_i8.data() + neg[ci+3]*cc16 + j));
